...
[hackthebox](/tags#hackthebox ) [htb-celestial](/tags#htb-celestial )
[ctf](/tags#ctf ) [nodejs](/tags#nodejs )
[deserialization](/tags#deserialization ) [htb-aragog](/tags#htb-aragog )
[pspy](/tags#pspy ) [cron](/tags#cron ) [oswe-like](/tags#oswe-like )  
...
python’s pickle, or in Node.js, deserialization of user input is almost always
a bad idea, and here’s we’ll show why. To escalate, we’ll take advantage of a
cron running the user’s code as root.

## Box Info

Name | [Celestial](https://hacktheboxltd.sjv.io/g1jVD9?u=https%3A%2F%2Fapp.hackthebox.com%2Fmachines%2Fcelestial) [ ![Celestial](../img/box-celestial.png)](https://hacktheboxltd.sjv.io/g1jVD9?u=https%3A%2F%2Fapp.hackthebox.com%2Fmachines%2Fcelestial)  
[Play on
HackTheBox](https://hacktheboxltd.sjv.io/g1jVD9?u=https%3A%2F%2Fapp.hackthebox.com%2Fmachines%2Fcelestial)  
---|---  
Release Date | 10 Mar 2018  
Retire Date | 04 May 2024  
OS | Linux ![Linux](../img/Linux.png)  
Base Points | Medium [30]  
Rated Difficulty | ![Rated difficulty for Celestial](../img/celestial-diff.png)  
Radar Graph | ![Radar chart for Celestial](../img/celestial-radar.png)  
![First Blood User](../img/first-blood-user.png) | 00:19:29[![yuntao](https://www.hackthebox.com/badge/image/12438) yuntao](https://app.hackthebox.com/users/12438)  
  
![First Blood Root](../img/first-blood-root.png) | 00:40:15[![arkantolo](https://www.hackthebox.com/badge/image/1183) arkantolo](https://app.hackthebox.com/users/1183)  
  
Creator | [![3ndG4me](https://www.hackthebox.com/badge/image/9652) 3ndG4me](https://app.hackthebox.com/users/9652)  
...
It is easy to make the page crash by messing with the parameters, giving it
...
Next we’ll create `exploit.js`, which will create the serialized payload by
putting some wrapping around the shell we just created:
...
This is the same format used in the blog post, though I added the slicing in
the last line to add the “()”, where as he added it manually to the output.
...
Finally, we’ll base64 encode that to get to the format of the cookie. I’ll use
`tail -n +2` to drop the first line:
...
Looking at the file modify times, it’s clear that output.txt is very recently
updated. In fact, it’s being updated every 5 minutes:
...
In the Documents directory next to user.txt, there’s a script.py that creates
that output:
...
There are enough clues on this host that simple enumeration should tip us off
to the cron. But, in other cases, we won’t be so lucky. This is a cool chance
to show off an awesome tool, `pspy`. I gave a quick overview on how to install
it in the [Aragog write-up](/2018/07/21/htb-aragog.html) . But let’s run it
here just for kicks.

After using `python3 -m http.server 80` and wget from Celestial to move the
pspy64 binary to the /tmp dir, we’ll run it. After spitting out a bunch of
errors and then info on all the running processes, it stops and waits. When
the 5 minute point comes around, here’s what we see:
...
So, hijack that file. Create a python rev shell one liner. Save the original
script so we can put it back when we’re done, and put ours into place:
...
We can see how if there’s no cookie, it sets one and sends “404”, and how if
there is, it uses `unserialize` and then reads values to make a string.

### cron

As root, we can see the cron job that is running `script.py`, and then copying
the `script.py` from `/root` to reset it to it’s original value:
...
The cron also sets the timestamps on `script.py` to match `user.txt`, so that
it’s not obvious that it’s being update regularly. To do this, it uses `date
-R -r`, which returns the modification time on `user.txt` in RFC 5322 format.
This is passed as an argument to `touch`, where the `-d` flag allows us to set
the timestamp instead of using the current time.

[](/2018/08/25/htb-celestial.html)
